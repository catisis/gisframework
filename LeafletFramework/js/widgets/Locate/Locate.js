        ...
        onAdd: function (map) {
            ...
            L.DomEvent
                ...
                .on(this._link, 'click', function () {
                    ...
                    this._map.getBounds().contains(this._event.latlng) || !this.options.setView || this._isOutsideMapBounds());
...
VitoGIS.Locate = function (_this) {
    ...
    _this.mapManager.map.on('startfollowing', function () {
        _this.mapManager.map.on('dragstart', lc._stopFollowing, lc);
    }).on('stopfollowing', function () {
        _this.mapManager.map.off('dragstart', lc._stopFollowing, lc);
    });

    _this.mapManager.map.on('locationfound', function (e) {
        //this.mapManager.map.removeMarker();
        if (!marker) {
            ...
            this.mapManager.map.flyTo(e.latlng, 17);
            this.mapManager.map.once('moveend', function (e) {
                marker.bounce(3);
            })
        }
    }, _this);
